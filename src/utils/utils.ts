...
export const airdropSol = async (connection: Connection, destPubKey: PublicKey, solAmount: number) => {
    const tx = await connection.confirmTransaction(
        await connection.requestAirdrop(
            destPubKey,
            anchor.web3.LAMPORTS_PER_SOL * solAmount,
        ),
        'confirmed',
    );

    console.log('airdropSol => ', tx)
}
